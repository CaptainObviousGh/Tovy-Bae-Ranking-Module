...
local Plugin = function(...)
	...
	local pluginLevel = 2
	local pluginUsage = "<User(s)> <RankId>" -- leave blank if the command has no arguments
	local pluginDescription = "Sets rank of a user using Tovy's API."
	...
	local function pluginFunction(Args) -- keep the name of the function as "pluginFunction"
		...
		for i,v in pairs(Args) do
			if v ~= Args[1] and v ~= Args[2] and v ~= Args[3] then
				if Reason == '' then
					Reason = v
				else
					Reason = Reason..' '..v
		...
		if Args[3] then
			...
			for a,b in next,Victims do
				require(game.ServerScriptService.TovyRankingLoader.TovyModule).setrank(b.UserId, Reason)
			end
		end
	end
